...
\begin{simbolos}
    ...
    \item[$ C(P, Q) $]  Função de custo de Entropia Cruzada entre a distribuição espacial de alta e baixa dimensão
    \item[$ p_{ij} $]   Probabilidade matemática de duas células serem vizinhas no espaço original de alta dimensão
    \item[$ q_{ij} $]   Probabilidade matemática de duas células estarem posicionadas próximas no espaço reduzido
\end{simbolos}
...
A análise preditiva de tumores lida com um volume massivo de informações simultâneas. No caso do câncer de mama, o algoritmo avalia cerca de trinta características diferentes de uma única vez, como o raio, a textura e a simetria celular \cite{Street1993}. Embora o computador consiga calcular padrões estatísticos em um espaço de trinta dimensões com enorme facilidade, o cérebro humano não consegue compreender e visualizar tais volumes de dados. Para resolver esse obstáculo visual e permitir que o médico enxergue o comportamento dos dados, utiliza-se técnicas matemáticas de redução de dimensionalidade. O objetivo dessas técnicas é comprimir as trinta variáveis em apenas dois eixos principais, criando um mapa plano que não perca a essência da informação original.

Entre os diversos métodos disponíveis, o algoritmo UMAP, sigla em inglês para Aproximação e Projeção de Variedade Uniforme, tornou-se uma das ferramentas mais eficientes da computação moderna. Em vez de simplesmente esmagar os dados, o UMAP utiliza conceitos visuais de vizinhança e topologia. Primeiro, ele observa os pacientes no espaço original de trinta dimensões e constrói uma complexa teia matemática, conectando os tumores que são anatomicamente muito parecidos. Em seguida, o desafio do algoritmo é desenhar essa mesma teia em uma tela de duas dimensões, garantindo que os tumores que eram vizinhos no espaço complexo continuem próximos no gráfico plano \cite{McInnes2018}.

Esse esforço de preservação cria um resultado visual de altíssimo valor clínico. Ao projetar os dados com essa cautela geométrica, o UMAP naturalmente agrupa os pacientes com biologia semelhante em blocos bem definidos, formando o que a literatura chama de agrupamentos ou \textit{clusters}. Na prática oncológica, isso significa que ao final do processamento o médico verá uma nuvem de pontos representando os tumores benignos em um lado do gráfico, e outra nuvem isolada representando os tumores malignos no lado oposto. Essa organização fornece uma prova clara e visual de que o sistema conseguiu encontrar padrões consistentes para separar a doença.

\subsection{Fundamentação Matemática: A Otimização Topológica}

Para que o computador consiga desenhar esse mapa de forma precisa, o UMAP traduz a ideia de vizinhança para uma equação de probabilidade. Inicialmente, o sistema calcula a chance, chamada de $p_{ij}$, de dois tumores serem idênticos no mundo real de trinta dimensões. Depois, ele calcula a chance $q_{ij}$ desses mesmos dois tumores estarem desenhados perto um do outro no novo gráfico de duas dimensões. O objetivo do modelo é fazer com que essas duas chances sejam o mais iguais possível. Para medir se está acertando ou errando durante o desenho, o algoritmo utiliza uma função de custo conhecida como Entropia Cruzada, definida pela Equação \ref{eq:umap_ce} \cite{McInnes2018}:
...
Apesar da aparência complexa, essa fórmula matemática funciona na prática exatamente como um sistema virtual de molas e elásticos para organizar os dados. A primeira metade da equação age como um elástico de atração. Se dois tumores são muito parecidos na vida real, resultando em um $p_{ij}$ alto, mas o algoritmo comete o erro de desenhá-los longe um do outro no gráfico final, gerando um $q_{ij}$ baixo, essa parte da fórmula cria uma penalidade matemática gigantesca. Para fugir dessa penalidade de erro, o computador é obrigado a puxar os dois pontos para perto.

Por outro lado, a segunda metade da equação age como uma mola de repulsão. O foco desse trecho da fórmula são os tumores que possuem anatomias completamente diferentes. Se o algoritmo acabar sobrepondo um tumor inofensivo em cima de um caso grave, a equação entra em ação e empurra os pontos para longe um do outro. O sistema repete esse processo de atração e repulsão milhares de vezes, ajustando as coordenadas até que todos os pontos estejam em perfeito equilíbrio, gerando a visualização final do mapa oncológico.
