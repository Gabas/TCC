...
% TODO: Descrever a estrutura em dois componentes — módulo científico
% (Wisconsin.ipynb) e módulo de software (pasta app / main.py). Explicar que
% os dois se comunicam por meio do arquivo serializado wisconsin.pkl.
% Incluir um diagrama de blocos com o fluxo: dados CSV → notebook →
% arquivo .pkl → aplicação → relatório gerado ao profissional de saúde.
...
O módulo científico é centralizado no \textit{notebook} \texttt{Wisconsin.ipynb}.
Este componente engloba todo o \textit{pipeline} experimental do projeto,
...
e padronização \textit{Z-score} das características morfológicas. Além disso, é neste
ambiente que ocorre o treinamento e a avaliação dos algoritmos de aprendizado
de máquina supervisionado nos conjuntos de treino e teste, bem como a configuração
dos modelos de Inteligência Artificial Explicável acoplados via bibliotecas
\texttt{shap} e \texttt{umap-learn}.

Por outro lado, o módulo de software compreende a aplicação de interface gráfica desktop 
estruturada dentro do diretório \texttt{app}, tendo o script \texttt{main.py} como ponto de entrada 
principal do sistema. Construído sobre a biblioteca CustomTkinter, este componente gerencia a experiência 
do usuário final, operando as telas de consulta, dashboards de métricas e a lógica de processamento em lote. 
Para mitigar erros humanos e otimizar a rotina de trabalho em ambientes clínicos, a aplicação foi projetada 
para receber diretamente arquivos de planilhas formatadas, eliminando a necessidade de inserção manual 
dos trinta atributos preditivos por parte do operador.

A comunicação e a transferência de inteligência entre esses dois ambientes independentes ocorrem por 
meio do arquivo serializado \texttt{wisconsin.pkl}, gerado através da biblioteca \texttt{pickle}. Este artefato 
estático encapsula os modelos preditivos previamente treinados, os parâmetros ajustados do 
padronizador \textit{Z-score} (\texttt{StandardScaler}) e a lista ordenada dos nomes dos atributos morfológicos. 
Dessa forma, o módulo de software realiza inferências em tempo real de modo puramente local, sem a 
necessidade de retreinar os algoritmos ou expor os dados a servidores externos, garantindo desempenho 
computacional e privacidade à infraestrutura de saúde.
 
O fluxo completo de processamento e a respectiva transição dos dados entre as camadas lógica e operacional da arquitetura integrada encontram-se detalhados na Figura~\ref{fig:fluxograma_solucao}.
...
Lembrete: Tirar marca d'água do fluxograma

% % ------------------------------------------------------------------------------
% \section{Base de Dados e Ambiente de Desenvolvimento}
% \label{sec:base-ambiente}
 
% % TODO: Descrever o arquivo data.csv (base WDBC, 569 amostras, 32 colunas
% % originais). Detalhar as decisões de limpeza: remoção da coluna 'id'
% % (sem valor preditivo) e da coluna 'Unnamed: 32' (artefato de exportação CSV).
% % Registrar a decisão de fixar o ambiente em Python 3.11.15, justificando a
% % restrição de compatibilidade com a aplicação main.py.
% % Apresentar uma tabela ou lista das bibliotecas principais e suas funções:
% % pandas/numpy (manipulação), scikit-learn (modelos, pré-processamento e
% % métricas), umap-learn (redução dimensional), shap (explicabilidade),
% % matplotlib/seaborn (visualizações) e pickle (serialização).
