...
% % TODO: Apresentar parágrafo introdutório descrevendo o pipeline de
% % pré-processamento como um todo, antes de detalhar cada etapa.
 
% \subsection{Codificação da Variável Alvo}
% \label{subsec:codificacao-alvo}
 
% % TODO: Descrever o mapeamento 'B' → 0 (Benigno) e 'M' → 1 (Maligno),
% % transformando a variável categórica em binária numérica para os classificadores.
 
% \subsection{Divisão Treino e Teste}
% \label{subsec:divisao-treino-teste}
 
% % TODO: Detalhar a partição estratificada com stratify=y, proporção 75/25
% % (426 amostras de treino, 143 de teste) e semente RANDOM_STATE = 42.
% % Justificar o uso de stratify: manter a proporção original de classes
% % (357 benignos / 212 malignos) em ambos os conjuntos, evitando viés
% % de amostragem.
 
% \subsection{Padronização Z-score}
% \label{subsec:padronizacao}
 
% % TODO: Descrever o StandardScaler ajustado exclusivamente no conjunto de
% % treino para evitar data leakage. Explicar que o mesmo scaler é aplicado
% % a três versões da base: X_train_scaled, X_test_scaled e X_scaled_all
% % (base completa, usada nas visualizações e no SHAP). Mencionar quais
% % modelos exigem padronização: Regressão Logística, SVM, PCA e UMAP.
 
% \subsection{Divisão Legada para a Árvore de Decisão}
% \label{subsec:divisao-legada}
 
% % TODO: Explicar a existência de um segundo split (80/20, sem padronização),
% % exclusivo para a Árvore de Decisão utilizada como baseline. Justificar
% % a manutenção desse split para preservar os resultados originais obtidos
% % antes da refatoração do notebook.
