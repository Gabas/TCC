...
Após submeter o conjunto de teste ao algoritmo, é fundamental quantificar o seu desempenho clínico. Na classificação binária, a base para qualquer avaliação é a construção de uma Matriz de Confusão. Essa ferramenta organiza os resultados das previsões em quatro categorias possíveis. Os Verdadeiros Positivos representam os tumores malignos corretamente identificados, enquanto os Verdadeiros Negativos são os tumores benignos confirmados. Por outro lado, os erros são divididos em Falsos Positivos, que ocorrem quando um tumor benigno gera um alarme falso de malignidade, e Falsos Negativos, que acontecem quando o sistema ignora um tumor maligno e o classifica como inofensivo. Em um cenário oncológico, o falso negativo representa a falha mais crítica do modelo, pois resulta em um paciente doente que é liberado sem receber o tratamento adequado.

A métrica mais intuitiva extraída dessa matriz é a acurácia, que representa a taxa geral de acertos do modelo. Contudo, confiar exclusivamente na acurácia é um erro metodológico perigoso na área da saúde devido ao problema matemático das classes desbalanceadas. Considere uma situação onde a base de dados possui noventa e nove pacientes saudáveis para cada paciente com câncer. Se um algoritmo quebrado simplesmente classificar todas as pessoas do mundo como saudáveis, ele obterá uma acurácia de noventa e nove por cento. Apesar de parecer um sucesso estatístico absoluto, o modelo seria clinicamente inútil e letal para a pequena parcela de pacientes doentes \cite{Faceli2011}.

Para contornar essa ilusão estatística, o desempenho deve ser avaliado por métricas específicas que observam os erros separadamente. A sensibilidade, também conhecida na literatura internacional como \textit{recall}, mede a capacidade do algoritmo de rastrear e encontrar todos os pacientes que realmente estão doentes. Em paralelo, a precisão avalia a confiabilidade técnica do alerta, ou seja, de todos os tumores que o sistema afirmou serem malignos, qual proporção realmente era. Como frequentemente existe uma balança entre essas duas métricas, onde o aumento de uma pode diminuir a outra, utiliza-se a métrica \textit{F1-score}. Esse indicador calcula a média harmônica entre a precisão e a sensibilidade, fornecendo um único número que atesta o equilíbrio e a segurança geral do classificador.
...
Este estudo serve como justificativa teórica para a escolha de algoritmos mais complexos neste TCC. Ele reforça a necessidade de evoluir além da estatística básica para capturar as nuances morfológicas que definem a malignidade celular, validando a abordagem tecnológica adotada.
