...
O fluxo completo de processamento e a respectiva transição dos dados entre as
...
\cite{Street1993}. Aproveitando a mesma etapa, duas colunas são removidas da base por não
agregarem valor preditivo ao sistema, conforme descrito na
Seção~\ref{sec:base-ambiente}. O Código~\ref{cod:codificacao} apresenta as
operações de limpeza e codificação realizadas sobre o DataFrame bruto.
...
entre as classes — 357 tumores benignos (62,7\%) e 212 malignos (37,3\%) — seja
preservada em ambos os subconjuntos. Sem essa estratificação, uma partição
...
produzindo uma estimativa artificialmente otimista do desempenho real do
modelo \cite{Faceli2011}.
...
de decisão são algoritmos invariantes à escala dos atributos, pois suas regras de
divisão baseiam-se em limiares de valor absoluto, não em distâncias ou gradientes
\cite{Faceli2011}. A manutenção dessa partição separada preserva os resultados
obtidos nas etapas iniciais de desenvolvimento do projeto, anteriores à
refatoração do \textit{pipeline} principal, garantindo a rastreabilidade e a
reprodutibilidade do histórico experimental.
...
A primeira etapa da EDA consistiu na inspeção estrutural do \textit{DataFrame} por meio
dos métodos \textit{info()} e \textit{describe()} da biblioteca pandas. O método info() confirmou
que a base, após a remoção das colunas não preditivas, contém 569 amostras e 31
colunas, a variável alvo e os trinta atributos morfológicos numéricos, sem
a presença de valores ausentes em nenhuma das colunas, dispensando etapas de
inserção de dados.

O método \textit{describe()} forneceu as estatísticas descritivas de cada atributo,
...
entre os atributos: variáveis como \textit{area\_worst} operam em escalas na casa dos
milhares, enquanto atributos como \textit{symmetry\_mean} assumem valores entre 0 e 1.
...
que a acurácia isolada se torne uma métrica enganosa, na qual um classificador trivial
que sempre predisse benigno atingiria 62,7\% de acurácia sem aprender nenhum
padrão real. Por essa razão, a avaliação dos modelos prioriza métricas como
Sensibilidade, F1-score e PR-AUC, que mensuram o desempenho sobre a classe
minoritária de forma mais fiel \cite{Davis2006}.
...
as regiões de maior magnitude. Em alguns atributos, como \textit{radius\_mean} e
...
aprender a lidar com essa variabilidade natural \cite{Faceli2011}.
