...
% Acurácia
A métrica mais intuitiva extraída dessa matriz é a acurácia, que representa a taxa geral de acertos do modelo. Contudo, confiar exclusivamente na acurácia é um erro metodológico perigoso na área da saúde devido ao problema matemático das classes desbalanceadas. Considere uma situação onde a base de dados possui noventa e nove pacientes saudáveis para cada paciente com câncer. Se um algoritmo quebrado simplesmente classificar todas as pessoas do mundo como saudáveis, ele obterá uma acurácia de 99\%. Apesar de parecer um sucesso estatístico absoluto, o modelo seria clinicamente inútil e letal para a pequena parcela de pacientes doentes \cite{Faceli2011}. Matematicamente, a acurácia é calculada pela razão entre as predições corretas e o total de predições, conforme a Equação \ref{eq:acuracia}. Seu resultado é expresso em uma escala de 0 a 1, onde valores próximos a 1 indicam maior acerto global.
\begin{equation}
    \label{eq:acuracia}
...
% Precisão
Em paralelo, a precisão (\textit{precision}) avalia a confiabilidade técnica do alerta, ou seja, de todos os tumores que o sistema afirmou serem malignos, qual proporção realmente era, expressada pela Equação \ref{eq:precisao} \cite{Davis2006}.
...
% ROC e AUC
A avaliação final do modelo é consolidada visualmente pela Curva de Característica de Operação do Receptor, amplamente referenciada pela sigla em inglês ROC. Essa curva ilustra graficamente a relação entre a taxa de acertos em pacientes doentes e a taxa de alarmes falsos em pacientes saudáveis. Para resumir essa visualização em uma nota clara, calcula-se a área sob a curva, denominada métrica AUC. O valor da AUC varia de zero a um, sendo que um resultado igual a um indica um classificador estatisticamente perfeito, capaz de separar totalmente os tumores benignos dos malignos. Por oferecer essa visão ampla do comportamento do modelo independentemente do equilíbrio dos dados, a AUC é considerada a métrica de ouro para comparar diferentes algoritmos oncológicos \cite{Faceli2011}. 

Adicionalmente, em cenários médicos onde o desbalanceamento das classes é crítico, a literatura moderna recomenda a utilização do gráfico de Curva Precisão-Revocação (\textit{Precision-Recall Curve}). Enquanto a Curva ROC plota a Sensibilidade no eixo y contra a taxa de falsos positivos no eixo x, a Curva PR plota a Precisão contra a Sensibilidade. O cálculo integral sob este gráfico gera a métrica PR-AUC (\textit{Area Under the PR Curve}). Ao contrário da ROC-AUC, a métrica PR-AUC ignora o grande volume de pacientes saudáveis (Verdadeiros Negativos), focando o seu rigor estatístico exclusivamente na capacidade do algoritmo em classificar a doença com exatidão. Uma pontuação PR-AUC elevada indica que o sistema consegue identificar quase todos os tumores malignos sem disparar alarmes falsos, o que a torna indispensável na validação oncológica \cite{Davis2006}.
